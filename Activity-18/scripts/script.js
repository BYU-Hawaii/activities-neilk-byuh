document.getElementById('registrationForm').addEventListener('submit', function(event) {
    ...
    var phoneValid = /^\+?[0-9]{1,4}?[-. \s]?[0-9]{1,3}?[-. \s]?[0-9]{3,4}[-. \s]?[0-9]{3,4}[-. \s]?[0-9]{0,9}$/.test(phone);//phone should be a phone number
    ...
    document.getElementById('phoneFeedback').textContent = phoneValid ? '' : 'Phone should be phone.';
